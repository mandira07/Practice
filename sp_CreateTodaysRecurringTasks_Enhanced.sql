...
CREATE OR ALTER PROCEDURE sp_CreateTodaysRecurringTasks_Enhanced
        ...
        BEGIN TRY
            ...
            WHERE rtd.TaskDate = @TargetDate
                ...
                AND NOT EXISTS (
                    ...
                    WHERE existing.TaskName LIKE t.TaskName + ''%'' + FORMAT(@TargetDate, ''yyyy-MM-dd'') + ''%''
            ...
            BEGIN
                ...
                SELECT 
                    t.TaskId,
                    t.TaskName + '' - '' + FORMAT(@TargetDate, ''yyyy-MM-dd'') as ProposedTaskName,
                    t.TaskDescription,
            ...
            BEGIN
                ...
                SELECT 
                    t.TaskName + '' - '' + FORMAT(@TargetDate, ''yyyy-MM-dd'') as TaskName,
                    t.TaskDescription + '' (Auto-created '' + FORMAT(GETDATE(), ''yyyy-MM-dd HH:mm'') + '')'',
...
-- 1. Process today's recurring tasks (actual execution)
-- EXEC sp_CreateTodaysRecurringTasks_Enhanced;

-- 2. Dry run to see what would be created today
...
-- EXEC sp_CreateTodaysRecurringTasks_Enhanced @ProcessDate = '2025-01-20', @TenantId = 10, @DryRun = 1;